...
#     def forward(self, x, x_mask, g):
#         # print("encoder Shape of x:", x.shape, "Shape of x_mask:", x_mask.shape, "Shape of g:", g.shape) # encoder Shape of x: torch.Size([114, 204, 320]) Shape of x_mask: torch.Size([114, 1, 320]) Shape of g: torch.Size([114, 128, 80])
#         x = self.pre_net(x)
#         x_mask = x_mask[:, :, ::np.prod(self.strides)][:, :, :x.shape[-1]]
#         x = x * x_mask
#         x = self.wn(x, x_mask, g) * x_mask
...
#     def forward(self, x, x_mask, g):
#         # print("decoder Shape of x:", x.shape, "Shape of x_mask:", x_mask.shape, "Shape of g:", g.shape)# Shape of x: torch.Size([114, 1240, 80]) Shape of x_mask: torch.Size([114, 1, 320]) Shape of g: torch.Size([114, 128, 320])
#         x = self.pre_net(x)
#         x = x * x_mask
#         x = self.wn(x, x_mask, g) * x_mask
#         x = self.out_proj(x)
...
#         # print('latent_size:', latent_size, 'hidden_channels:', hidden_channels, 'in_out_channels:', in_out_channels, 'kernel_size:', kernel_size, 'dec_n_layers:', dec_n_layers, 'gin_channels:', gin_channels, 'strides:', strides)
...
#         self.spatial_attention = SpatialAttention(latent_size, hidden_channels // 8) # div8
...
#         :return:
#         """
#         # print('x', x.shape) # x torch.Size([114, 320, 204])
#         x_mask = x_mask[:, None, :] # [B, 1, T]
#         g = g.transpose(1,2) # [B, C_g, T]
#         g_for_sqz = g
...
#             z_q, m_q, logs_q, x_mask_sqz = self.encoder(x, x_mask, g_sqz)
#             # print("Shape of z_q:", z_q.shape) # Shape of z_q: torch.Size([114, 16, 80])
#             z_q = self.spatial_attention(z_q)
#             # print("Shape of att z_q:", z_q.shape) # Shape of z_q: [114, 16, 80])
#             x_recon = self.decoder(z_q, x_mask, g)
...
#     def forward(self, x):
#         # x: [B, C, T]
#         query = self.query_conv(x)  # [B, C//8, T]  
#         key = self.key_conv(x)  # [B, C//8, T]
#         value = self.value_conv(x)  # [B, C, T]

#         # Compute attention scores
#         scores = torch.bmm(query.transpose(1, 2), key)  # [B, T, T]
#         attention_weights = self.softmax(scores)

#         # print('query', query.shape, 'key', key.shape, 'value', value.shape, 'attention_weights', attention_weights.shape) # div 4 is query torch.Size([114, 64, 80]) key torch.Size([114, 64, 80]) value torch.Size([114, 16, 80]) attention_weights torch.Size([114, 80, 80])
#         # div 8 is query torch.Size([114, 32, 80]) key torch.Size([114, 32, 80]) value torch.Size([114, 16, 80]) attention_weights torch.Size([114, 80, 80])

#         # Apply attention weights
#         attended = torch.bmm(value, attention_weights.transpose(1, 2))  # [B, C, T]
#         return attended
...
# class PitchContourVAEModel(nn.Module):
#     def __init__(self, hparams, in_out_dim=64, audio_in_dim=1024, sqz_prior=False, cond_drop=False, use_prior_flow=True, num_emotions=8, emotion_embedding_dim=128):
#         super().__init__()
#         self.hparams = copy.deepcopy(hparams)
#         feat_dim = 128
#         self.audio_in_dim = audio_in_dim
...
#         # emotion
#         self.emotion_embedding = EmotionEmbedding(num_emotions, feat_dim)
...
#         cond_dim = feat_dim + feat_dim + feat_dim + feat_dim  
...
#         # emotion feature
#         emotion_labels = batch['emotion'].to(self.device) # int tensor, train [512,1] , [1,1] infer
#         emotion_embed = self.emotion_embedding(emotion_labels)
#         # print(f'emotion_embed shape: {emotion_embed.shape}') # emotion_feat shape: torch.Size([1, 1, 128])
#         emotion_embed = emotion_embed.expand(-1, cond_feat.size(1), -1)
#         # print(f'emotion_embed shape: {emotion_embed.shape}') # emotion_feat shape: torch.Size([1, 296, 128])

#         emotion_feat = self.emotion_encoder(emotion_embed.transpose(1,2)).transpose(1,2)
#         # print(f'emotion_feat shape: {emotion_feat.shape}') # emotion_feat shape: 

#         cond_feats = [cond_feat, pitch_feat, blink_feat, emotion_feat]
#         cond_feat = torch.cat(cond_feats, dim=-1)  
#         cond_feat = self.cond_proj(cond_feat)
...
#     def forward(self, x, x_mask, g):
#         # print("decoder Shape of x:", x.shape, "Shape of x_mask:", x_mask.shape, "Shape of g:", g.shape)# decoder Shape of x: torch.Size([114, 16, 80]) Shape of x_mask: torch.Size([114, 1, 320]) Shape of g: torch.Size([114, 128, 320])
#         x = self.pre_net(x)
#         x = x * x_mask
#         x = self.wn(x, x_mask, g) * x_mask
#         x = self.out_proj(x)
...
# class FVAE0(nn.Module):
#     def __init__(self,
#                  in_out_channels=64, hidden_channels=256, latent_size=16,
#                  kernel_size=3, enc_n_layers=5, dec_n_layers=5, gin_channels=80, strides=[4,],
#                  use_prior_glow=True, glow_hidden=256, glow_kernel_size=3, glow_n_blocks=5,
#                  sqz_prior=False, use_pos_emb=False):
#         super(FVAE0, self).__init__()
#         self.in_out_channels = in_out_channels
#         self.strides = strides
#         self.hidden_size = hidden_channels
#         self.latent_size = latent_size
...
#             for i, s in enumerate(strides)
#         ])
#         self.encoder = FVAEEncoder0(in_out_channels, hidden_channels, latent_size, kernel_size,
#                                    enc_n_layers, gin_channels, strides=strides)
#         if use_prior_glow:
#             self.prior_flow = ResidualCouplingBlock(
#                 latent_size, glow_hidden, glow_kernel_size, 1, glow_n_blocks, 4, gin_channels=gin_channels)
...
#             self.value_proj = nn.Linear(latent_size, hidden_channels)
#             if self.in_out_channels in [7, 64]:
#                 self.decoder = FVAEDecoder0(hidden_channels, hidden_channels, in_out_channels, kernel_size,
#                                     dec_n_layers, gin_channels, strides=strides)
#             elif self.in_out_channels == 71:
#                 self.exp_decoder = FVAEDecoder0(hidden_channels, hidden_channels, 64, kernel_size,
#                                     dec_n_layers, gin_channels, strides=strides)
#                 self.pose_decoder = FVAEDecoder0(hidden_channels, hidden_channels, 7, kernel_size,
#                                     dec_n_layers, gin_channels, strides=strides)
#             if self.use_pos_embed:
#                 self.embed_positions = SinusoidalPositionalEmbedding(self.latent_size, 0,init_size=2000+1,)
#         else:
#             self.decoder = FVAEDecoder0(latent_size, hidden_channels, in_out_channels, kernel_size,
#                                     dec_n_layers, gin_channels, strides=strides)
...
# class FVAE_emo(nn.Module):
#     def __init__(self,
#                  in_out_channels=64, hidden_channels=256, latent_size=16,
#                  kernel_size=3, enc_n_layers=5, dec_n_layers=5, gin_channels=80, strides=[4,],
#                  use_prior_glow=True, glow_hidden=256, glow_kernel_size=3, glow_n_blocks=5,
#                  sqz_prior=False, use_pos_emb=False):
#         super(FVAE_emo, self).__init__()
#         self.in_out_channels = in_out_channels
#         self.strides = strides
#         self.hidden_size = hidden_channels
#         self.latent_size = latent_size
...
# class PitchContourVAEModel_emo(nn.Module):
#     def __init__(self, hparams, in_out_dim=64, audio_in_dim=1024, sqz_prior=False, cond_drop=False, use_prior_flow=True, num_emotions=8, emotion_embedding_dim=128):
#         super().__init__()
#         self.hparams = copy.deepcopy(hparams)
#         feat_dim = 128
...
#         # self.emotion_projector = nn.Linear(emotion_embedding_dim, feat_dim)  # Projecting to the same dimension as audio features
#         self.emotion_encoder = nn.Sequential(*[
#                 nn.Conv1d(feat_dim, feat_dim , 3, 1, 1, bias=False),
#                 nn.BatchNorm1d(feat_dim),
#                 nn.GELU(),
...
#         self.sqz_prior = sqz_prior
#         self.use_prior_flow = use_prior_flow
#         self.vae = FVAE_emo(in_out_channels=in_out_dim, hidden_channels=256, latent_size=16, kernel_size=5,
#             enc_n_layers=8, dec_n_layers=4, gin_channels=feat_dim, strides=[4,],
#             use_prior_glow=self.use_prior_flow, glow_hidden=64, glow_kernel_size=3, glow_n_blocks=4,sqz_prior=sqz_prior)
#         self.downsampler = LambdaLayer(lambda x: F.interpolate(x.transpose(1,2), scale_factor=0.5, mode='nearest').transpose(1,2))
...
#         emotion_labels = batch['emotion'].to(self.device) # int tensor, train [512,1] , [1,1] infer
#         emotion_embed = self.emotion_embedding(emotion_labels)
#         # print(f'emotion_embed shape: {emotion_embed.shape}') # emotion_embed shape: torch.Size([1, 1, 16])
#         # emotion_embed = self.emotion_projector(emotion_embed) 
#         # print(f'emotion_embed shape: {emotion_embed.shape}') # emotion_feat shape: torch.Size([1, 1, 128])
#         emotion_embed = emotion_embed.expand(-1, cond_feat.size(1), -1)
#         # print(f'emotion_embed shape: {emotion_embed.shape}') # emotion_feat shape: torch.Size([1, 296, 128])
...
#         cond_feats = [cond_feat, pitch_feat, blink_feat, emotion_feat] # add emotion feature
...
#             x=exp
#             # x = exp[:,:,:64]
#             # print(f'x shape: {x.shape}') # x shape: torch.Size([114, 320, 64])
#             x_recon, loss_kl, z_p, m_q, logs_q = self.vae(x=x, x_mask=mask, g=cond_feat, infer=False)
#             x_recon = x_recon * mask.unsqueeze(-1)
#             ret['pred'] = x_recon
#             ret['mask'] = mask
...
# class PitchContourVAEModel2(nn.Module):
#     def __init__(self, hparams, in_out_dim=64, audio_in_dim=1024, sqz_prior=False, cond_drop=False, use_prior_flow=True):
#         super().__init__()
#         self.hparams = copy.deepcopy(hparams)
#         feat_dim = 128
...
#         self.sqz_prior = sqz_prior
#         self.use_prior_flow = use_prior_flow
#         self.vae = FVAE0(in_out_channels=in_out_dim, hidden_channels=256, latent_size=16, kernel_size=5,
#             enc_n_layers=8, dec_n_layers=4, gin_channels=feat_dim, strides=[4,],
#             use_prior_glow=self.use_prior_flow, glow_hidden=64, glow_kernel_size=3, glow_n_blocks=4,sqz_prior=sqz_prior)
#         self.downsampler = LambdaLayer(lambda x: F.interpolate(x.transpose(1,2), scale_factor=0.5, mode='nearest').transpose(1,2))
