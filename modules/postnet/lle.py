...
def solve_LLE_projection_batch(feat, feat_base):
    ...
    if K == 1:
        ...
        # when matrix ATA is singular, meaning it cannot be inverted because it has a zero on its diagonal or it is not full rank:
        epsilon = 1e-6  # Small value to add to the diagonal
        identity_matrix = torch.eye(ATA.size(-1), device=ATA.device)  # Identity matrix of the same size
        regularized_ATA = ATA + epsilon * identity_matrix
        inv_ATA = torch.inverse(regularized_ATA)  # [N, K-1, K-1]
