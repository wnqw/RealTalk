...
class AudioAttNet(nn.Module):
    # Audio feature attention-based smoother in AD-NeRF
    def __init__(self, in_out_dim=64, seq_len=3):
    ...
    def forward(self, x):
        ...
        """
       # 3,64
        y = x[:, :self.in_out_dim].permute(1, 0).unsqueeze(0)  # [b, c] => [1, c, b], 1,64,3
        y = self.attentionConvNet(y) # [1,1,b], 1,1,3
        y = y.view(1, self.seq_len) # 1,3

        y = self.attentionNet(y).view(self.seq_len, 1) # [8, 1];  
    ...
    #     return: [c]
    #     """
    #     # Let b = x.shape[0]. We do not assume it's 8 anymore.
    #     b, c = x.shape

    #     # Constrain c so it doesn’t exceed in_out_dim, in case x has more channels
    #     x = x[:, :self.in_out_dim]    # shape: [b, in_out_dim]

    #     # Permute to shape [1, in_out_dim, b] so that 'b' is the "sequence" dimension
    #     y = x.permute(1, 0).unsqueeze(0)  # shape: [1, in_out_dim, b]

    #     y = self.attentionConvNet(y)      # shape: [1, 1, b]

    #     # Now flatten out [1, 1, b] => [1, b], then apply linear+softmax => [1, b]
    #     # Then reshape to [b, 1]
    #     y = self.attentionNet(y.view(1, b)).view(b, 1)

    #     # Weighted sum
    #     #   [b, 1] * [b, c] => [b, c] => sum over b => [c]
