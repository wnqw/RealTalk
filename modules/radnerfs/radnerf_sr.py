...
class RADNeRFwithSR(NeRFRenderer):
    def __init__(self, hparams):
        ...
        # self.brow_embedding = nn.Embedding(1,  self.cond_out_dim//2)
        # self.brow_encoder = nn.Sequential(
        #     *[
        #         nn.Linear(self.cond_out_dim//2, self.cond_out_dim//2),
        #         nn.Linear(self.cond_out_dim//2, hparams['eyebrows_dim']),
        #     ]
        # )
    ...
    def cal_cond_feat(self, cond, eye_area_percent=None):
        ...
        # brow_feat = self.brow_embedding(torch.tensor(0, device=cond_feat.device)).reshape([1, -1])
        # brow_feat = self.brow_encoder(brow_feat)
        # cond_feat[..., :hparams['eyebrows_dim']] = cond_feat[..., :hparams['eyebrows_dim']] + brow_feat.expand(cond_feat[..., :hparams['eyebrows_dim']].shape)
