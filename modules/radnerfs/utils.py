...
def get_audio_features(features, att_mode, index):
    ...
    if att_mode == 0:
        ...
        half = smo_win_size // 2   # 1
        left = index - half    # index - 1
        right = index + (smo_win_size - half)  # index + 2

        # clamp if out of bounds
        left = max(0, left)
        right = min(features.shape[0], right)

        auds = features[left:right]  # shape: [?, 1, 204]

        # If we got fewer than 3 frames, pad up to 3
        current_len = auds.shape[0]
        if current_len < smo_win_size:
            # number of frames we need
            needed = smo_win_size - current_len
            pad_shape = list(auds.shape)
            pad_shape[0] = needed
            pad = torch.zeros(pad_shape, device=auds.device, dtype=auds.dtype)
            auds = torch.cat([auds, pad], dim=0)  # shape now [3, 1, 204]
