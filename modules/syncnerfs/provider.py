...
from .utils import get_rays, get_bg_coords, AudDataset
...
class SyncNeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        binary_file_name = os.path.join(self.root_path, "trainval_dataset.npy")
        ...
        # print(f'idexp_lm3d_normalized {idexp_lm3d_normalized.shape}') [6073, 68, 3])
        idexp_lm3d_normalized_win = idexp_lm3d_normalized.reshape([-1, 1, self.keypoint_num * 3])
        # print(f'conds {idexp_lm3d_normalized_win.shape}') ([6073, 1, 204])
        # sys.exit()
        self.conds = idexp_lm3d_normalized_win

        if self.type == 'train':
            self.conds = self.conds[:len(frames)]
        else:
            self.conds = self.conds[-len(frames):]
    ...
    def collate(self, index):
        ...
        results['conds'] = get_audio_features(self.conds, att_mode=2, index=index[0]) # 3,1,204
