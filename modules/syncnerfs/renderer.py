...
# from .raymarching as raymarching
...
class SyncNeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, cond, bg_coords, poses, eye=None, index=0, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        enc_a = self.cal_cond_feat(cond) # [1, 32]
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        conds = get_audio_features(self.conds, self.att, rand_idx).to(self.density_bitfield.device)

        enc_a = self.cal_cond_feat(conds)
    ...
    def render_torso(self, rays_o, rays_d, cond, bg_coords, poses, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # auds: [B, 29, 16]
        # eye: [B, 1]
        # bg_coords: [1, N, 2]
