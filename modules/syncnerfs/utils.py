...
class LMDMeter:
    ...
    def get_landmarks(self, img):
        ...
        if self.backend == 'dlib':
            ...
            lms = self.predictor.get_landmarks(img)
            if lms is None:
                return []
            lms = lms[-1]
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        eye = data['eye'] # [B, 1]
        conds = data['conds'].to(self.device) # torch.Size([3, 1, 204])
        index = data['index'] # [B]
        loss_perception =0
        ...
        if not self.opt.torso:
            outputs = self.model.render(rays_o, rays_d, conds, bg_coords, poses, eye=eye, index=index, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
        else:
            outputs = self.model.render_torso(rays_o, rays_d, conds, bg_coords, poses, eye=eye, index=index, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
    ...
    def eval_step(self, data):
        ...
        conds = data['conds'].to(self.device)
        index = data['index'] # [B]
        eye = data['eye'] # [B, 1]
        ...
        outputs = self.model.render(rays_o, rays_d, conds, bg_coords, poses, eye=eye, index=index, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        conds = data['conds'].to(self.device) # [B, 29, 16]
        index = data['index']
        H, W = data['H'], data['W']
        ...
        outputs = self.model.render(rays_o, rays_d, conds, bg_coords, poses, eye=eye, index=index, staged=True, bg_color=bg_color, perturb=perturb, **vars(self.opt))
    ...
    def test(self, loader, save_path=None, name=None, write_image=False):
        ...
        # write video
        all_preds = np.stack(all_preds, axis=0) #(1, 512, 512, 3)
        save_vid=os.path.join(save_path, f"{name}.mp4")

        writer = imageio.get_writer(save_vid, fps = 25, format='FFMPEG', codec='h264')
        for i in tqdm.trange(len(all_preds), desc=f"imageio"):
            writer.append_data(all_preds[i])
        writer.close()

        if self.opt.aud != '':
            os.system(f'ffmpeg -i {save_vid} -i {self.opt.aud} -c:v copy -c:a aac {os.path.join(save_path, f"{name}_audio.mp4")} -y')
            ...
            for data in loader:    
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    try:
                        ...
                        print(f"data[conds]: {data['conds'].shape}")
                loss_val = loss.item()
                total_loss += loss_val
