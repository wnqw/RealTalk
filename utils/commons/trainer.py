...
class Trainer:
    ...
    def fit(self, task_cls):
        try:
            if self.use_ddp:
                ...
                # File "/mnt/bn/ailabrenyi/entries/yezhenhui/projects/RealTalk_private/venv_113/lib/python3.9/site-packages/torch/nn/modules/batchnorm.py", line 735, in forward
                # world_size = torch.distributed.get_world_size(process_group)
                # to address the error in batchNorm using venv_113
                # self.init_ddp_connection_tcp(0, 1) 
                self.task = task_cls()
                self.task.trainer = self
                setproctitle.setproctitle(f'RealTalk_worker ({hparams["work_dir"]})')
                self.run_single_process(self.task)
        except:
            traceback.print_exc()
            time.sleep(5)
            subprocess.check_call(f'pkill -f "RealTalk_worker \({hparams["work_dir"]}"', shell=True)
        return 1
    ...
    def ddp_run(self, gpu_idx, task_cls, hparams_):
        ...
        setproctitle.setproctitle(f'RealTalk_worker ({hparams_["work_dir"]}_{gpu_idx})')
